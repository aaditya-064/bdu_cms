  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-gray-500 text-sm mt-1">Activity history and security events</p>
      </div>
      ...
      {loading && logs.length === 0 ? (
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-gray-100 bg-gray-50">
                    ...
                    <th className="text-left text-xs font-medium text-gray-500 px-4 py-3 hidden md:table-cell">Resource</th>
                ...
                <tbody>
                  {logs.map((log) => (
                    <tr key={log._id} className="border-b border-gray-50 hover:bg-gray-50">
                      ...
                      <td className="px-4 py-3 text-sm text-gray-700">{getUserName(log)}</td>
                      <td className="px-4 py-3">
                        <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium ${
                          log.action.includes('DELETE') ? 'bg-red-100 text-red-700' :
                          log.action.includes('REVEAL') ? 'bg-yellow-100 text-yellow-700' :
                          log.action.includes('CREATE') || log.action.includes('IMPORT') || log.action.includes('UPLOAD') ? 'bg-green-100 text-green-700' :
                      ...
                      <td className="px-4 py-3 text-sm text-gray-400 hidden lg:table-cell font-mono text-xs">
                        {log.ipAddress || '-'}
                      </td>
                    </tr>
                  ))}
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-12 text-center">
          ...
          <p className="text-gray-500">Activity will be recorded here as actions are performed.</p>
        </div>
      )}
    </div>
  );
